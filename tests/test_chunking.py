...
class TestChunking(unittest.TestCase):
    ...
    @patch('src.infrastructure.adapters.chunk_stores.chroma_chunk_store.GoogleGenerativeAIEmbeddings')
    def test_chunking_use_case(self, mock_embeddings):
        mock_embeddings.return_value.embed_documents.return_value = [[0.1, 0.2, 0.3]] * 100
        ...
        retrieved_chunk = self.chunk_store.get(f"{chunks[0].metadata.get('source', 'doc')}_{chunks[0].metadata.get('chunk_index', 0)}")
