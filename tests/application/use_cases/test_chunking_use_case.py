...
def chunking_use_case(mock_document_loader, mock_chunk_store):
    return ChunkingUseCase(document_loader=mock_document_loader, chunk_store=mock_chunk_store)

def test_chunking_use_case_execute(chunking_use_case, mock_document_loader, mock_chunk_store):
    mock_document_loader.load.return_value = [Document(content="test", metadata={})]
    mock_chunk_store.add.return_value = None
    ...
    assert len(chunks) > 0
    mock_chunk_store.add.assert_called()
